...
const get3days = () => {
  ...
  if (currentDay > 3) {
    ...
    if (cpt < 3 || forecastDays.length < 3)
    ...
    for (let index = currentDay + 1; index < currentDay + 4; index++) {
      forecastDays.push(index);
    }
};
...
const weatherData = async (lat, lon) => {
  try {
    const response = await fetch(
      `https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}&units=metric&appid=${weatherApiKey}`
    );
    const json = await response.json();
    console.log(json);
...
const fillWeatherData = (data) => {
  ...
  document.getElementById("temp_top").innerText =
    Math.round(currentTemp) + "°C";
  document.getElementById("condition_top").innerText =
  ...
  for (let index = 0; index < 3; index++) {
    ...
    document.getElementById(`forecast_temp_${index}`).innerText =
      Math.round(forcastedTemp) + "°C";
    document.getElementById(`forecast_day_${index}`).innerText =
      weekday[forecastDays[index]];
    document.getElementById(`forecast_weather_${index}`).innerText =
